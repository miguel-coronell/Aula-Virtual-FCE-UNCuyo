...
function buildFileButton(file){
  ...
  return `
    <a class="btn-pdf ${isPending ? "needs-link" : ""}"
       href="${isPending ? "#" : file.url}"
       target="_blank" rel="noopener"
       data-filename="${file.label}"
       ${isPending ? 'data-pending="true"' : ""}>
      ${fileIcon(file.type)}
      <span>${isPending ? "⚠ " : ""}${file.type === "ppt" ? "Descargar PPTX" : "Descargar PDF"}</span>
    </a>`;
}
...
function buildBiblioItem(item){
  ...
  return `
    <a class="biblio-item ${isPending ? "" : ""}" href="${isPending ? "#" : item.url}" target="_blank" rel="noopener" ${isPending ? 'data-pending="true"' : ""}>
      <span class="biblio-item__icon">${ICON.doc}</span>
      <span class="biblio-item__text">
        <strong>${item.label}</strong>
        <span>${isPending ? "⚠ Falta enlace de Google Drive" : "Abrir en Google Drive"}</span>
      </span>
    </a>`;
}
